...
// Raspberry Pi sends: l or r continuously
// side stores the destination direction only.
// Returning logic is handled by get_turn_direction().
...
a:
   ...
   /* if (sum == 0) {
      ...
      serial_delay(node_delay);
      reading();
      if (sum != 0) cross = get_turn_direction();
      m1 = millis();
    }
